...
```bash
poetry run poe create-sagemaker-role
```
...
```bash
poetry run poe create-sagemaker-execution-role
```
...
```bash
poetry run poe run-training-pipeline
```
...
```bash
poetry run poe run-evaluation-pipeline
```
...
```bash
poetry run poe deploy-inference-endpoint
```
To test it out, run:
```bash
poetry run poe test-sagemaker-endpoint
```
To delete it, run:
```bash
poetry run poe delete-inference-endpoint
```
...
```bash
poetry run poe run-generate-instruct-datasets-pipeline
```

Generate the preference dataset:
```bash
poetry run poe run-generate-preference-datasets-pipeline
```

Run all of the above compressed into a single pipeline:
```bash
poetry run poe run-end-to-end-data-pipeline
```
...
```bash
poetry run poe run-export-data-warehouse-to-json
```
...
```bash
poetry run poe run-import-data-warehouse-from-json
```

Export ZenML artifacts to JSON:
```bash
poetry run poe run-export-artifact-to-json-pipeline
```
...
1. Collect data: `poetry run poe run-digital-data-etl`

2. Compute features: `poetry run poe run-feature-engineering-pipeline`

3. Compute instruct dataset: `poetry run poe run-generate-instruct-datasets-pipeline`

4. Compute preference alignment dataset: `poetry run poe run-generate-preference-datasets-pipeline`
...
5. SFT fine-tuning Llamma 3.1: `poetry run poe run-training-pipeline`

6. For DPO, go to `configs/training.yaml`, change `finetuning_type` to `dpo`, and run `poetry run poe run-training-pipeline` again

7. Evaluate fine-tuned models: `poetry run poe run-evaluation-pipeline`
...
8. Call only the RAG retrieval module: `poetry run poe call-rag-retrieval-module`

9. Deploy the LLM Twin microservice to SageMaker: `poetry run poe deploy-inference-endpoint`

10. Test the LLM Twin microservice: `poetry run poe test-sagemaker-endpoint`

11. Start end-to-end RAG server: `poetry run poe run-inference-ml-service`

12. Test RAG server: `poetry run poe call-inference-ml-service`
